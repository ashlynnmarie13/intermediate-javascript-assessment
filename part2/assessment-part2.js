...
function noWeakLink() {
  return (
    $http({
      ...
      // CODE HERE...
      .then(obj => {
        firstUser = obj.data[0];
        return obj;
      })
      .then(obj => {
        thirdUser = obj.data[2];
        return obj.data[9];
      })
  );
}
...
function frodo(startingHungerValue, startingDangerValue) {
  var secondBreakfast = startingHungerValue;
  var afternoonTea = startingDangerValue;

  return {
    dinnerOverFire: function() {
      return {
        hunger: (secondBreakfast -= 25),
        danger: (afternoonTea += 40)
      };
    },
    hidingInBush: function() {
      return {
        hunger: (secondBreakfast += 35),
        danger: (afternoonTea -= 20)
      };
    }
  };
}
